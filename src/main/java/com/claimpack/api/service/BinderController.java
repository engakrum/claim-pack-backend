...
@RestController
@RequestMapping("/api")
@CrossOrigin(origins = "*") // ALLOWS CONNECTION FROM UI
public class BinderController {
    ...
    public ResponseEntity<byte[]> uploadFiles(
        ...
        try {
            ...
            // 4. List Files (Table of Contents)
            document.add(new Paragraph("Attached Evidence:", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 14)));
            ...
            for (MultipartFile file : files) {
                if (file.getContentType() != null && file.getContentType().startsWith("image")) {
                    try {
                        ...
                        document.add(new Paragraph("[Could not display image: " + file.getOriginalFilename() + "]"));
                    }
                }
            }
